...
## Setup Docker Valhalla

A livello implementativo si è scelto di ospitare il motore routing Valhalla in locale tramite Docker, in tal modo è possibile mantenere un controllo totale sui dati, maniopolando il grafo stradale e definendo regole di routing specifiche (evita particolare nodo).

Tramite immagine Docker, Valhalla gestisce in autonomia il download dei dati grezzi (.osm.pbf) e la compilazione delle *routing tiles* (il grafo stradale ottimizzato).

Attualmente il routing è funzionante solo sulle regioni: *Trentino-Alto Adige*, *Veneto*, *Friuli-Venezia Giulia*. In futuro il supporto verrà esteso a tutta l'Italia.

Per avviare il container automaticamente scaricare l'estratto cartografico aggiornato del Nord-Est Italia e il setup dei *tiles* stradali operato automaticamente da Valhalla eseguire dalla root di progetto:

```bash
docker compose -f docker-compose.valhalla.yaml up -d 
```

Come accennato Valhalla avvierà automaticamente la costruzione del grafo stradale. L'operazione può richiedere alcuni minuti. Per verficare lo stato di avanzamento dell'operazione eseguire:

```bash
docker logs -f urban-access-valhalla
```

Al completamento dell'operazione, se avvenuta con successo, il seguente comando deve ritornare come primo parametro la versione del software (es.: `"version":"3.5.1"`).

```bash
curl "http://localhost:8002/status" 
```

La serie di campi riotrnati successivamente rappresentano tutti gli endpoint API esposti dal servizio che ora il server in docker possiede (es.: `route` usato pr calcolare il percorso da A a B).

Il processo ha automaticamente creato nella root di progetto la cartella `valhalla_data`.
All'interno di essa recarsi al file `valhalla.json` e assicurarsi che il blocco `"service_limits"` sia correttamente configurato nel seguente modo:

```json
urban-access/valhalla_data/valhalla.json
...
"service_limits": {
    "auto": {
...
```bash
docker restart urban-access-valhalla
```

Verificare il funzionamento della chiamata provando per esempio:
...
-d '{
  "locations": [
    {"lat": 46.0678, "lon": 11.1214},
    {"lat": 46.0716, "lon": 11.1266}
  ],
  "costing": "auto"
}'
```
