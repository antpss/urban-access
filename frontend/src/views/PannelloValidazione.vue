<template>
  <!-- Blocco contestuale innestato nel popup della mappa (via Teleport).
       Mostra stato, avanzamento verso la soglia e il bottone di conferma. -->
  <div class="rounded-2xl border border-slate-200 bg-white p-5 w-[280px]">
    ...
    <p class="text-sm text-slate-600 mb-3">{{ report.descrizione }}</p>

    <!-- Barra di avvicinamento alla soglia: solo mentre è in verifica -->
    <div v-if="report.stato === 'IN_VERIFICA'" class="mb-4">
      ...
      <p class="text-[11px] text-slate-500 mt-1.5 font-medium">
        {{ scoreCorrente }} / {{ soglia }} punti di affidabilità raccolti
      </p>
    </div>

    <!-- Esito (successo o errore) -->
    <div v-if="esito"
      class="text-sm rounded-xl p-3 mb-3 font-medium"
    ...
    <button
      type="button"
      @click="conferma"
      :disabled="!puoVotare || isLoading"
      class="w-full py-3 font-bold rounded-xl text-white bg-gradient-to-r from-emerald-500 to-teal-500
      ...
      <span v-else-if="report.stato === 'APERTA'">Già validata dalla community</span>
      <span v-else-if="isAutore">Non puoi validare la tua segnalazione</span>
      <span v-else-if="haVotato">Hai già confermato</span>
...
const props = defineProps({
  // segnalazione privata già caricata dalla mappa; deve avere _id, stato, autore, scoreAssociato, categoria, descrizione
  report: { type: Object, required: true }
});

// il parent (Mappa) reagisce per aggiornare marker e barra
const emit = defineEmits(['validated', 'score-updated']);
...
// score aggregato gestito localmente per feedback immediato della barra
const scoreLocale = ref(props.report.scoreAssociato ?? 0);

const soglia = computed(() => props.report.sogliaValidazione || 10);
const scoreCorrente = computed(() => scoreLocale.value);
...
// (Il JWT usa `userId`, ma l'oggetto salvato lato client espone `_id`.)
const myId = user?._id;
const isAutore = computed(() => String(props.report.autore) === String(myId));
...
const conferma = async () => {
  ...
  try {
    ...
    if (response.status === 201 || response.status === 200) {
      ...
      haVotato.value = true;
      esitoOk.value = false;
      esito.value = data.error || 'Hai già validato questa segnalazione.';
