        ...
        <div class="relative w-full max-w-lg bg-white rounded-[2rem] shadow-2xl p-8 max-h-[90vh] overflow-y-auto z-50">
          ...
          <div v-if="serverError" class="bg-rose-50 border-l-4 border-rose-500 p-4 mb-6 text-sm text-rose-700 rounded-r-xl">
            ...
            <ul v-if="serverErrorDetails.length > 0" class="list-disc ml-5 mt-1 font-medium">
              <li v-for="(detail, index) in serverErrorDetails" :key="index">
                {{ detail.message }}
              </li>
            </ul>
          </div>
          ...
          <form @submit.prevent="submitSegnalazione" class="space-y-5">
            ...
            <div>
              ...
              <textarea v-model="form.descrizione" @input="validateDescrizione" required rows="3" placeholder="Descrivi il problema (min. 10 caratteri)..."
                ...
                >
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                      d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6M9 7h6m-7 0h8m-1-2a1 1 0 00-1-1h-2a1 1 0 00-1 1l-.293 1h4.586L15 5z" />
                  </svg>
                </button>
              </div>

              <p v-if="form.foto.length > 0" class="text-[11px] text-slate-500 mt-2 ml-1 font-medium">
                {{ form.foto.length }} foto selezionate
              </p>

              <p v-if="fileError" class="text-rose-500 text-[11px] mt-1.5 ml-1 font-semibold">
                {{ fileError }}
              </p>
            </div>
            ...
            <div class="pt-4 flex flex-wrap gap-3">
              ...
              <button type="submit" :disabled="!isFormValid || isLoading"
                class="flex-[2] min-w-[160px] py-3.5 font-bold rounded-xl text-white bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600
                  disabled:from-slate-300 disabled:to-slate-300 disabled:text-slate-500 disabled:shadow-none disabled:opacity-50 disabled:cursor-not-allowed shadow-lg transition-all">
...
const validateLatitudine = () => {
  if (form.value.latitudine !== null && form.value.latitudine !== '') {
    if (form.value.latitudine < -90 || form.value.latitudine > 90) {
      latitudineError.value = 'Latitudine deve esserecompresa tra -90 e 90.';
    } else {
      latitudineError.value = '';
    }
  } else {
...
const validateLongitudine = () => {
  if (form.value.longitudine !== null && form.value.longitudine !== '') {
    if (form.value.longitudine < -180 || form.value.longitudine > 180) {
      longitudineError.value = 'Longitudine deve essere compresa tra -180 e 180.';
    } else {
      longitudineError.value = '';
    }
  } else {
...
const isFormValid = computed(() => {
  return (descrizioneError.value === '' &&
        ...
        form.value.latitudine !== null &&
        form.value.latitudine !== '' &&
        form.value.longitudine !== null &&
        form.value.longitudine !== '');
});

const handleFileChange = (event) => {
  const files = Array.from(event.target.files);
  if (files.length > 5) {
    fileError.value = 'max. 5 foto consentite';
...
const submitSegnalazione = async () => {
  ...
  try {
    const response = await authFetch(`${API_BASE_URL}/reports/public`, {
      method: 'POST',
      body: formData
    });
    ...
    if (!response.ok) {
      const errorObject = new Error(data.error || data.message || 'Errore sconosiuto durante l\'invio della segnalazione.');
      errorObject.details = data.details || [];
      throw errorObject;
    }
